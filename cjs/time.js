...
function parseDateValue(value) {
    ...
    if (tools_1.is.date(value)) {
        return value;
    }
    if (tools_1.is.text(value)) {
        const matches = value.match(/^(\d{4})-?(\d{2})-?(\d{1,2})$/);
        if (tools_1.is.array(matches)) {
 ...
 */
function methodName(unit, isUTC = false) {
 ...
 * @param atStartOf
 */
exports.copyAndSetTime = (unit, basis, atStartOf = true) => {
...
exports.absFloor = (n) => n < 0 ? Math.ceil(n) || 0 : Math.floor(n);
function zoneText(zoneOffset) {
...
class DateTime {
    ...
    boundary(unit, atStartOf = true) {
        ...
        switch (unit) {
            ...
            case 60000 /* Minute */:
            case 1000 /* Second */:
                return exports.copyAndSetTime(unit, this, atStartOf);
            default:
                return this.clone();
        }
    }
     ...
     */
    set(unit, value) {
        return tools_1.is.number(value) ? this.clone().update(unit, value) : this;
    }
    /**
    ...
    format(pattern = 'YYYY-MM-DDTHH:mm:ssZ') {
        return pattern.replace(/Y{2,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|m{1,2}|s{1,2}|Z{1,2}/g, match => {
            switch (match) {
                ...
                case 'MM':
                    return tools_1.leadingZeros(this.month + 1, 2);
                case 'MMM':
                    return tools_1.month[this.month].slice(0, 3);
                case 'MMMM':
                    return tools_1.month[this.month];
                case 'D':
                    return String(this.dayOfMonth);
                case 'DD':
                    return tools_1.leadingZeros(this.dayOfMonth, 2);
                case 'd':
                    return String(this.dayOfWeek);
                case 'dddd':
                    return tools_1.weekday[this.dayOfWeek];
                case 'H':
                    return String(this.hour);
                case 'HH':
                    return tools_1.leadingZeros(this.hour, 2);
                case 'm':
                    return String(this.minute);
                case 'mm':
                    return tools_1.leadingZeros(this.minute, 2);
                case 's':
                    return String(this.second);
                case 'ss':
                    return tools_1.leadingZeros(this.second, 2);
                case 'Z':
    ...
    diff(other, unit = 1 /* Millisecond */, precise = false) {
        ...
        let result = exports.monthsApart(this, other);
        switch (unit) {
            case 220752000000 /* Year */:
                result /= 12;
                break;
        ...
        return precise ? result : exports.absFloor(result);
    }
    /**
 ...
 */
exports.dateTime = (dateValue) => new DateTime(dateValue);
